 ...
 * Full-featured Word Processor client action for Odoo 17
 */
...
export class WordEditorAction extends Component {
    ...
    async _autoSave() {
        ...
        try {
            const result = await jsonrpc("/word_editor/autosave", {
    ...
    async openTemplates() {
        try {
            const tpls = await jsonrpc("/word_editor/templates", {});
...
// Odoo's action manager injects these extra props into every client action.
