...
class ResUsers(models.Model):
    ...
    def _check_credentials(self, password, env):
        # Odoo 17 still passes only the password to this hook. The web login
        # request carries the selected method in the hidden ``type`` field.
        auth_type = request.params.get('type', 'password') if request else 'password'
        hash_field = {
            'pin': 'login_pin_hash',
        ...
        if password and stored_hash and self._crypt_context().verify(password, stored_hash):
            return None
        raise AccessDenied()
